...
assert.equal(comfortStack.nextPatchLabel, "Comfort Bundle", "comfort risk should create a serial subtle repair bundle");
assert.ok(comfortStack.nextPatchItems.length >= 2, "comfort bundle should combine multiple light repair moves");
assert.ok(Number(comfortPatch.deEss || 0) > Number(comfortBase.deEss || 0), "comfort bundle should raise de-ess for sibilance");
assert.ok(Number(comfortPatch.consonantSoftness || 0) > Number(comfortBase.consonantSoftness || 0), "comfort bundle should smooth mouth/transient density");
assert.ok(Number(comfortPatch.dryWet || 100) < Number(comfortBase.dryWet || 100), "comfort bundle should add a wet/dry guard when the stack is fatiguing");
